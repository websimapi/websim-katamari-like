...
export class CityGenerator {
  constructor(scene, world) {
    ...
    // Biome distribution is controlled by noise
    this.biomeNoise = this.createSimplexNoise(12345);
    this.terrainNoise = this.createSimplexNoise(54321);
  }
  
  createSimplexNoise(seed) {
    // Simple pseudo-random generator
    const random = function(x) {
      const n = (seed + x) % 233280;
      return ((n * 9301 + 49297) % 233280) / 233280;
    };
    
    // Create a noise function that returns a value between -1 and 1
    return function(x, z) {
      ...
      for (let i = 0; i < 4; i++) {
        value += amplitude * (random(nx * frequency + nz * frequency * 100) * 2 - 1);
        max += amplitude;
        amplitude *= 0.5;
        frequency *= 2;
      }
  ...
  getBiomeForPosition(x, z) {
    ...
    const noiseValue = this.biomeNoise(x, z);
